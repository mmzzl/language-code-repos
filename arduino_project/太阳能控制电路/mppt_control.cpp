...
#include <TFT_eSPI.h>               // using the ST7789 library for ESP32-C3
...
void setup()                           // run once, when the sketch starts
{
  ...
  // display the constant stuff on the TFT
  tft.setCursor(0, 0);
  tft.print("SOL");
  tft.setCursor(40, 0);
  tft.print("BAT");
...
void tft_display()
{
  ...
  if (current_backlight_state != back_light_pin_State) {
    ...
    if (back_light_pin_State == HIGH)
      tft.setBacklight(true);// finish with backlight on
    else
...
void backLight_timer(){
  if((millis() - time) <= 15000)         // if it's been less than the 15 secs, turn the backlight on
      ...
      tft.setBacklight(false);           // if it's been more than 15 secs, turn the backlight off
}
void spinner(void) {
  static int cspinner;
  static char spinner_chars[] = { '*','*', '*', ' ', ' '};
  cspinner++;
  tft.print(spinner_chars[cspinner%sizeof(spinner_chars)]);
}
